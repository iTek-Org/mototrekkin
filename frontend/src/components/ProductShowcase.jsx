...
const ProductCard = ({ image, name, href }) => (
  <a href={href} className="bg-white p-4 rounded-lg shadow-md text-center flex-shrink-0 block no-underline transition-all duration-400 ease-in-out transform scale-100 hover:scale-110 hover:shadow-lg">
    ...
    <div className="bg-gray-900 text-white flex flex-col justify-center py-20">
      ...
      <div className="flex flex-col md:flex-row justify-center gap-6">
        {products.map((product, index) => (
          <ProductCard key={index} image={product.image} name={product.name} href={product.href} />
        ))}
      </div>
    </div>
  );
